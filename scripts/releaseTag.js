...
/**
 * Find the remote pointing to mui/mui.
 *
...
async function findMuiOrgRemote() {
  ...
  return remoteLines
    ...
    .find((remote) => {
      ...
      // - git@github.com:mui/mui.git
      // but not:
      // - git@github.com:mui/mui-docs.git
...
async function main(argv) {
  ...
  if (muiOrgRemote === undefined) {
    throw new TypeError(
      'Unable to find the upstream remote. It should be a remote pointing to "mui/mui". ' +
        'Did you forget to add it via `git remote add upstream git@github.com:mui/mui.git`? ' +
...
yargs
  .command({
    command: '$0',
    description: 'Tags the current release and pushes these changes to mui/mui.',
    builder: (command) => {
