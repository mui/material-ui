...
/**
 * Snapshots enhance{High,Medium,Low}Density into a checked-in EmitTarget
...
function renderRow(r: Row): string {
  ...
  return `  {
    ...
    values: { high: ${JSON.stringify(r.values.high)}, medium: ${JSON.stringify(
      r.values.medium,
    )}, low: ${JSON.stringify(r.values.low)} },
  },`;
}
