...
describe('Luxon utils', () => {
  let luxonUtil: LuxonUtils;
  let luxonUtilDe: LuxonUtils;
  ...
  it('Should return a Luxon DateTime instance', () => {
    ...
    const nowString = luxonUtil.date('2018-01-01');
    expect(now.constructor.name).toBe('DateTime');
    expect(nowJs.constructor.name).toBe('DateTime');
    expect(nowString.constructor.name).toBe('DateTime');
    expect(now.isValid).toBe(true);
    expect(nowJs.isValid).toBe(true);
    expect(nowString.isValid).toBe(true);
  });

  it('Should return a DateTime instance of now', () => {
    const invalid = luxonUtil.date(null);
    const now = luxonUtil.date(invalid.toISO());

    expect(invalid.constructor.name).toBe('DateTime');
    expect(invalid).toEqual(now);
  });
  ...
  it('Should add one day', () => {
    ...
    expect(then).toEqual(now.plus({ days: 1 }));
    expect(thenPast).toEqual(now.minus({ days: 1 }));
  });
  ...
  it('Should format numbers correclty', () => {
    ...
    expect(luxonUtil.formatNumber(num)).toBe('23');
  });
  ...
  it('Should set the year', () => {
    const now = luxonUtil.date(new Date(2018, 0));
    const plusOneYear = now.plus({ years: 1 });
