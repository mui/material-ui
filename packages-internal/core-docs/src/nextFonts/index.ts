...
// 400, 500 and 600 all render above the fold on every docs page (body copy,
// app bar and nav, section chips), so a late swap of any of them reflows the
// first viewport: Lighthouse reported CLS 0.018–0.021 from "web font loaded"
// with only 400 preloaded. 700 is a single "⌘K" badge and stays deferred.
...
const ibmPlexSansDeferred = localFont({
  declarations: [{ prop: 'font-family', value: 'IBM Plex Sans' }],
  preload: false,
  src: [{ path: './fonts/IBMPlexSans-Bold.woff2', weight: '700', style: 'normal' }],
});
