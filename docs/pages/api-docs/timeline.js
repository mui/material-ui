...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Timeline');
  const classDescriptions = mapApiTranslations(req2, 'Timeline');
  const classConditions = mapApiTranslations(req3, 'Timeline');
