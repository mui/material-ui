...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'TabList');
  const classDescriptions = mapApiTranslations(req2, 'TabList');
  const classConditions = mapApiTranslations(req3, 'TabList');
