...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Slide');
  const classDescriptions = mapApiTranslations(req2, 'Slide');
  const classConditions = mapApiTranslations(req3, 'Slide');
