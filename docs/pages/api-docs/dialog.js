...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Dialog');
  const classDescriptions = mapApiTranslations(req2, 'Dialog');
  const classConditions = mapApiTranslations(req3, 'Dialog');
