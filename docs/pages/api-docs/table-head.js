...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'TableHead');
  const classDescriptions = mapApiTranslations(req2, 'TableHead');
  const classConditions = mapApiTranslations(req3, 'TableHead');
