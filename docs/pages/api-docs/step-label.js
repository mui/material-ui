...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'StepLabel');
  const classDescriptions = mapApiTranslations(req2, 'StepLabel');
  const classConditions = mapApiTranslations(req3, 'StepLabel');
