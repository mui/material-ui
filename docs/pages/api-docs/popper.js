...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Popper');
  const classDescriptions = mapApiTranslations(req2, 'Popper');
  const classConditions = mapApiTranslations(req3, 'Popper');
