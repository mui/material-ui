...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'AppBar');
  const classDescriptions = mapApiTranslations(req2, 'AppBar');
  const classConditions = mapApiTranslations(req3, 'AppBar');
