...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'TimelineConnector');
  const classDescriptions = mapApiTranslations(req2, 'TimelineConnector');
  const classConditions = mapApiTranslations(req3, 'TimelineConnector');
