...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Input');
  const classDescriptions = mapApiTranslations(req2, 'Input');
  const classConditions = mapApiTranslations(req3, 'Input');
