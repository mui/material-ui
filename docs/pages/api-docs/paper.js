...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Paper');
  const classDescriptions = mapApiTranslations(req2, 'Paper');
  const classConditions = mapApiTranslations(req3, 'Paper');
