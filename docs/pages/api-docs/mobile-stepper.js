...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'MobileStepper');
  const classDescriptions = mapApiTranslations(req2, 'MobileStepper');
  const classConditions = mapApiTranslations(req3, 'MobileStepper');
