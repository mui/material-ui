...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Chip');
  const classDescriptions = mapApiTranslations(req2, 'Chip');
  const classConditions = mapApiTranslations(req3, 'Chip');
