...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Avatar');
  const classDescriptions = mapApiTranslations(req2, 'Avatar');
  const classConditions = mapApiTranslations(req3, 'Avatar');
