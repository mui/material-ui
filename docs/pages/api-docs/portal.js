...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Portal');
  const classDescriptions = mapApiTranslations(req2, 'Portal');
  const classConditions = mapApiTranslations(req3, 'Portal');
