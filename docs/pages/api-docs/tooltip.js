...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Tooltip');
  const classDescriptions = mapApiTranslations(req2, 'Tooltip');
  const classConditions = mapApiTranslations(req3, 'Tooltip');
