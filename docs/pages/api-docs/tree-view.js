...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'TreeView');
  const classDescriptions = mapApiTranslations(req2, 'TreeView');
  const classConditions = mapApiTranslations(req3, 'TreeView');
