...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Container');
  const classDescriptions = mapApiTranslations(req2, 'Container');
  const classConditions = mapApiTranslations(req3, 'Container');
