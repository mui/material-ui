...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Link');
  const classDescriptions = mapApiTranslations(req2, 'Link');
  const classConditions = mapApiTranslations(req3, 'Link');
