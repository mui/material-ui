...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'FormLabel');
  const classDescriptions = mapApiTranslations(req2, 'FormLabel');
  const classConditions = mapApiTranslations(req3, 'FormLabel');
