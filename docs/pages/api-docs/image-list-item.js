...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'ImageListItem');
  const classDescriptions = mapApiTranslations(req2, 'ImageListItem');
  const classConditions = mapApiTranslations(req3, 'ImageListItem');
