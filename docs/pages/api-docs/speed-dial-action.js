...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'SpeedDialAction');
  const classDescriptions = mapApiTranslations(req2, 'SpeedDialAction');
  const classConditions = mapApiTranslations(req3, 'SpeedDialAction');
