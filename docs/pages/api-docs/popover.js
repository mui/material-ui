...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Popover');
  const classDescriptions = mapApiTranslations(req2, 'Popover');
  const classConditions = mapApiTranslations(req3, 'Popover');
