...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Tabs');
  const classDescriptions = mapApiTranslations(req2, 'Tabs');
  const classConditions = mapApiTranslations(req3, 'Tabs');
