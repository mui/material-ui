...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'ImageListItemBar');
  const classDescriptions = mapApiTranslations(req2, 'ImageListItemBar');
  const classConditions = mapApiTranslations(req3, 'ImageListItemBar');
