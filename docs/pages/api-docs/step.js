...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Step');
  const classDescriptions = mapApiTranslations(req2, 'Step');
  const classConditions = mapApiTranslations(req3, 'Step');
