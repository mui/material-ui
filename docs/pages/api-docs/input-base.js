...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'InputBase');
  const classDescriptions = mapApiTranslations(req2, 'InputBase');
  const classConditions = mapApiTranslations(req3, 'InputBase');
