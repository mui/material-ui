...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Card');
  const classDescriptions = mapApiTranslations(req2, 'Card');
  const classConditions = mapApiTranslations(req3, 'Card');
