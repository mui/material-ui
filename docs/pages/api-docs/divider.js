...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Divider');
  const classDescriptions = mapApiTranslations(req2, 'Divider');
  const classConditions = mapApiTranslations(req3, 'Divider');
