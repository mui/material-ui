...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Stepper');
  const classDescriptions = mapApiTranslations(req2, 'Stepper');
  const classConditions = mapApiTranslations(req3, 'Stepper');
