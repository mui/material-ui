...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Collapse');
  const classDescriptions = mapApiTranslations(req2, 'Collapse');
  const classConditions = mapApiTranslations(req3, 'Collapse');
