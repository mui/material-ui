...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'ListItem');
  const classDescriptions = mapApiTranslations(req2, 'ListItem');
  const classConditions = mapApiTranslations(req3, 'ListItem');
