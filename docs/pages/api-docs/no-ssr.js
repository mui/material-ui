...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'NoSsr');
  const classDescriptions = mapApiTranslations(req2, 'NoSsr');
  const classConditions = mapApiTranslations(req3, 'NoSsr');
