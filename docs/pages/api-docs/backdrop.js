...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Backdrop');
  const classDescriptions = mapApiTranslations(req2, 'Backdrop');
  const classConditions = mapApiTranslations(req3, 'Backdrop');
