...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'CircularProgress');
  const classDescriptions = mapApiTranslations(req2, 'CircularProgress');
  const classConditions = mapApiTranslations(req3, 'CircularProgress');
