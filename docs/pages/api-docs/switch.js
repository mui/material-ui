...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Switch');
  const classDescriptions = mapApiTranslations(req2, 'Switch');
  const classConditions = mapApiTranslations(req3, 'Switch');
