...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'TableCell');
  const classDescriptions = mapApiTranslations(req2, 'TableCell');
  const classConditions = mapApiTranslations(req3, 'TableCell');
