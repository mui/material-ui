...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'List');
  const classDescriptions = mapApiTranslations(req2, 'List');
  const classConditions = mapApiTranslations(req3, 'List');
