...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'Fade');
  const classDescriptions = mapApiTranslations(req2, 'Fade');
  const classConditions = mapApiTranslations(req3, 'Fade');
