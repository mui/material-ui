...
| <span class="prop-name">clearIcon</span> | <span class="prop-type">node</span> | <span class="prop-default">&lt;ClearIcon fontSize="small" /></span> | The icon to display in place of the default clear icon. |
| <span class="prop-name">clearOnBlur</span> | <span class="prop-type">bool</span> | <span class="prop-default">!props.freeSolo</span> | If `true`, the input's text is cleared on blur if no value is selected.<br>Set to `true` if you want to help the user enter a new value. Set to `false` if you want to help the user resume his search. |
...
| <span class="prop-name">selectOnFocus</span> | <span class="prop-type">bool</span> | <span class="prop-default">!props.freeSolo</span> | If `true`, the input's text is selected on focus. It helps the user clear the selected value. |
...
If that's not sufficient, you can check the [implementation of the component](https://github.com/mui-org/material-ui/blob/next/packages/material-ui/src/Autocomplete/Autocomplete.js) for more detail.
