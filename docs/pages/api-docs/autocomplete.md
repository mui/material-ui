...
| <span class="prop-name">handleHomeEndKeys</span> | <span class="prop-type">bool</span> | <span class="prop-default">true</span> | If `true`, pressing the `Home` and `End` keys will highlight the first and last item respectively in the popup. If `false`, the `Home` and `End` keys will control the character position in the text input when the popup shows.   |
