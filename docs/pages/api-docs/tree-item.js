...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'TreeItem');
  const classDescriptions = mapApiTranslations(req2, 'TreeItem');
  const classConditions = mapApiTranslations(req3, 'TreeItem');
