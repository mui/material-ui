...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'MenuItem');
  const classDescriptions = mapApiTranslations(req2, 'MenuItem');
  const classConditions = mapApiTranslations(req3, 'MenuItem');
