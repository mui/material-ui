...
export async function getStaticProps() {
  ...
  const propDescriptions = mapApiTranslations(req, 'NativeSelect');
  const classDescriptions = mapApiTranslations(req2, 'NativeSelect');
  const classConditions = mapApiTranslations(req3, 'NativeSelect');
