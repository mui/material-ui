...
| `disableRipple`                                                    | same (default `false`)                  | The item root is a styled `ButtonBase`, so each item ripples like the classic `MenuItem`. Set `disableRipple` to turn the ripple off.                                                                                                                                                      |
| `focusVisibleClassName`, `onFocusVisible`, `action.focusVisible()` | `highlighted` class and data attributes | Set the style with CSS. The items opt out of the `theme.focusVisible` ring, because Base UI applies the highlight on hover and a `:focus-visible` ring never matches a hover. The classic `MenuItem` keeps the ring.                                                                       |
