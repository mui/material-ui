...
- **Separate components.** The parts that users change per instance stay separate: the items (`Menu2Item`, `Menu2LinkItem`), the checkbox and radio items with their indicators, the radio group, the groups with their labels, and the separator. A submenu trigger is not a component: you pass your own element to the `trigger` prop of `Menu2Submenu`, and only its class hooks (`menu2SubmenuTriggerClasses`) are public.
...
- **`trigger` takes an element at both levels.** Base UI's `render` merges the trigger behavior into that element, so the caller keeps their own component. A string is not valid. The two levels match, and a `Tooltip` wraps the trigger at either level:
...
- **A wrapper must forward the props and the ref to its child.** The `Tooltip` of Material UI does this. A wrapper that you write must do the same, or the trigger behavior does not reach the element.
...
- **The trigger does not infer `nativeButton`.** The caller declares it through `slotProps.trigger` when the trigger element is not a native button. The item, the checkbox item, and the radio item keep the inference: each derives `nativeButton` from its root slot and its `component` prop, and the `nativeButton` prop overrides that. The link item renders an `<a>`, so it has no `nativeButton` prop.
...
- **Open state.** The list that contains a submenu trigger styles that trigger, because the trigger is the caller's element. The list styles the hover, the highlighted, the open, the disabled, and the selected states. The list needs the `:hover` rule because Base UI marks the trigger as highlighted only after the submenu opens. A test pins the order: a selected trigger keeps its selected blend when its submenu opens.
...
2. Design phase for the API shape: **in progress**. The decisions above are settled against the playground and the recipes previews. The five open questions above are the remainder.
