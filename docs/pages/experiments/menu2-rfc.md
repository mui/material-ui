...
10. Behavior defaults divergence: initial focus on pointer-opened menus is **decided** -- keep the Base UI behavior (see the benchmark reading above). Still open for the other defaults: hover-open submenus (`openOnHover` on the submenu trigger, 100ms delay), and whether the non-modal scroll behavior should be the default.
