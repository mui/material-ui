...
- **Slot handling:** use `useSlot` for the popup list. At Base UI render boundaries, reuse `appendOwnerState`, `resolveComponentProps`, and Material UI's `mergeSlotProps`. Use Base UI's `mergeProps` for behavioral props and compose refs. Keep adapters local until another integration needs them. These adapters filter Material props from host elements, combine state-based classes, infer native buttons, and prevent duplicate keyboard activation.
...
- **Theme state:** keep `Mui-*` classes and `ownerState` as the Material customization contract. The checkbox item, the radio item, and the submenu trigger give their slots the live checked, open, or highlighted state. `Menu2Item` and `Menu2LinkItem` resolve their slot callbacks from static props; use the state classes to style them. Collapsed popup slot callbacks receive resolved public props, not live uncontrolled open state. Internal animation and placement styles can use Base UI attributes.
