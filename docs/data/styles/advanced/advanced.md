...
> If you don't want to have both Emotion & JSS in your bundle, please refer to the [`@mui/system`](/docs/data/system/getting-started/overview/overview.md) documentation which is the recommended alternative.
...
> This example creates a theme object for custom-built components. If you intend to use some of the MUI's components you need to provide a richer theme structure using the `createTheme()` method. Head to the [theming section](/docs/data/material/customization/theming/theming.md) to learn how to build your custom MUI theme.
...
You can [follow the server side guide](/docs/data/material/guides/server-rendering/server-rendering.md) for a more detailed example, or read the [`ServerStyleSheets` API documentation](/docs/data/styles/api/api.md#serverstylesheets).
...
The class names are generated by [the class name generator](/docs/data/styles/api/api.md#creategenerateclassnameoptions--class-name-generator).
...
- The `disableGlobal` option of the [class name generator](/docs/data/styles/api/api.md#creategenerateclassnameoptions--class-name-generator) is `false` (the default).
