...
> ⚠️ `@mui/styles`是MUI的_**传统**_样式解决方案。 它依赖于 [JSS ](https://cssinjs.org/)作为样式解决方案，在`@mui/material`中已经不使用了，在v5中被废弃。 如果你不想在你的包里同时有Emotion和JSS，请参考[`@mui/system`](/system/getting-started/overview/)文档，这是推荐的替代方案。
