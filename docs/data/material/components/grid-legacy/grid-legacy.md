...
The [grid](https://m2.material.io/design/layout/responsive-layout-grid.html) creates visual consistency between layouts while giving you flexibility across a wide variety of designs.
...
The `rowSpacing` and `columnSpacing` props let you specify the row and column gaps independently.
...
The `container` and `item` props are two independent booleans; they can be combined to let a Grid component be both a flex container and child.
