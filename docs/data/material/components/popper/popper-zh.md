...
- 为了避免渲染问题，子组件作为页面 body 的 [`Portal`](/material/components/portal/)。 您可以使用 `disablePortal` 来禁用此行为。
- 不同于  [`Popper`](/material/components/popover/) 组件，你可以自由实现滚动（scroll）行为。 弹出提示的位置会随着视口中的可用面积而更新。
- Clicking away 不会隐藏`弹出提示`组件。 若您需要这个功能，请使用 [`ClickAwayListener`](/material/components/click-away-listener/) - 可以参照 [menu 文档章节](/material/components/menus/#menulist-composition) 中的一个样例。
