...
Unless the `rows` prop is set, the height of the text field dynamically matches its content (using [TextareaAutosize](/material/components/textarea-autosize/)).
...
The `select` prop makes the text field use the [Select](/material/components/selects/) component internally.
