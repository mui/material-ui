...
export function Annotations({
  ...
  const reserveLane = (side: 'top' | 'bottom', centre: number, label: string) => {
  ...
  const nextRail = (side: Side, extra = 0, centre = 0, label = '') => {
    if (side === 'top') {
      return bounds.y - 22 - (reserveLane('top', centre, label) + extra) * 30;
    }
    if (side === 'bottom') {
      return (
        bounds.y + bounds.height + 22 + (reserveLane('bottom', centre, label) + extra) * 30
      );
    }
  ...
  const reserveY = (side: 'left' | 'right', y: number) => {
