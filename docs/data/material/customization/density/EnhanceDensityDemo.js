  ...
  React.useEffect(() => {
    ...
    const measure = () => {
      ...
      const edges = (prefix, suffix) => ({
        top: parseFloat(styles[`${prefix}Top${suffix}`]),
        right: parseFloat(styles[`${prefix}Right${suffix}`]),
        bottom: parseFloat(styles[`${prefix}Bottom${suffix}`]),
        left: parseFloat(styles[`${prefix}Left${suffix}`]),
      });
      setMetrics({
