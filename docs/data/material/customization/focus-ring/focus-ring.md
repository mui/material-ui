...
Set `focusVisible: true` on the theme to render a curated focus ring on every [ButtonBase](/material-ui/api/button-base/)-derived component when it receives **keyboard** focus (`:focus-visible`):
...
The ring is **opt-in** and non-breaking: when `focusVisible` is `undefined` or `false`, nothing changes.
...
```js
createTheme({ focusVisible: { boxShadow: '0 0 0 4px rgba(0, 0, 0, 0.4)' } });
```
