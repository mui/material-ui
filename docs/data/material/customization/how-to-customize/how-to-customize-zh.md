...
The easiest way to add style overrides for a one-off situation is to use the [`sx` prop](/system/basics/#the-sx-prop) available on all Material-UI components. 下面是一个示例： Here is an example: Here is an example:
...
你可以使用浏览器开发工具来确定您要覆盖的组件的插槽。 这样做可以节省你的很多时间。 Material-UI 注入到 DOM 中 的样式依赖于[遵循简单模式的类名](/styles/advanced/#class-names)：`[hash]-Mui[Component name]-[name of the slot]`.
