...
If you don't have [the complete and latest](/store/items/figma-react/) Material UI for Figma version installed, you can test the plugin by using [the Community version](https://www.figma.com/community/file/912837788133317724/material-ui-for-figma-and-mui-x/) instead.
...
Support for more components will follow soon.
:::
...
Connect will then generate the following theme code:
...
The generated theme only targets classes that correspond to the specific Switch configuration defined above, so styles will only be applied when that is matching.

To customize all other possible states, you need to go to each instance and apply the desired design chances. To do this more efficiently, follow these steps:
