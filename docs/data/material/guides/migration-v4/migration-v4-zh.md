...
- `createGenerateClassName` 功能不再从 `@mui/material/styles` 导出。 你应该直接从 `@mui/styles` 导入它。
  ...
  **不使用**`@mui/styles`生成自定义类名称，请参考[ClassNameGenerator](/material/guides/classname-generator/)。
...
- HOC已被移除。 有一个使用 [`useMediaQuery` 钩子函数](/material/components/use-media-query/#migrating-from-withwidth) 的替代方法。
...
我们有一个 [专用页面](/material/guides/pickers-migration/) 用于迁移 `@material-ui/pickers` 到 v5
...
- 此组件被废弃，因为它的功能可以使用 [`sx`](/system/basics/#the-sx-prop) 属性或 [`useMediaQuery`](/material/components/use-media-query/) 钩子替代。
...
- 重构CSS 以匹配最新的 [Material Design 指引](https://material.io/components/sliders) 并使自定义风格更加直观。 [查看文档](/material/components/slider/)。 <a href="/components/slider/#continuous-sliders"><img width="247" alt="" src="https://user-images.githubusercontent.com/3165635/121884800-a8808600-cd13-11eb-8cdf-e25de8f1ba73.png" style="margin: auto"></a>

  您可以降低滑块的密度，使用 [`size="small"` 属性](/material/components/slider/#sizes) 使效果更接近v4。
...
查看可用的[全局状态类名称](/material/customization/how-to-customize/#state-classes)列表。
