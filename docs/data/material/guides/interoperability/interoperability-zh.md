...
明确向提组件提供类名是不是太大费周章了？ [You can target the class names generated by MUI](/system/styles/advanced/#with-material-ui-core).
...
The [Portal](/material-ui/react-portal/) provides a first-class way to render children into a DOM node that exists outside the DOM hierarchy of the parent component. 当您使用这样的 styled-components 规范其 CSS 的方式时，可能会遇到一些无法附着样式的问题。
...
> If you are updating from `@material-ui/core` (v4) to `@mui/material` (v5) checkout [migration guide](/material-ui/guides/migration-v4/#2-use-tss-react).
