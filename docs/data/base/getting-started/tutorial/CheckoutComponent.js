...
const InputRoot = styled('div')(
  ...
  .${inputUnstyledClasses.focused} & {
    ...
    box-shadow: 0 0 0 3px ${theme.palette.mode === 'dark' ? grey[800] : blue[100]};
  }
`,
);
...
const CardTypes = styled(({ className, ...props }) => {
  const formControlContext = useFormControlUnstyledContext();
  ...
  return (
    <div {...props} className={clsx(className, brand && brand)}>
...
export default function CheckoutComponent() {
  ...
  return (
    <FormControlUnstyled value={cardNum} onChange={handleChange}>
      <Label>Card number</Label>
      <Input
        slots={{
          root: InputRoot,
        }}
        endAdornment={<CardTypes />}
      />
      <HelperText />
    </FormControlUnstyled>
  );
