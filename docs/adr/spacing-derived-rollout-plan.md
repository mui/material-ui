...
- [x] **SwitchBase `padding: 9`** (internal — used by Checkbox + Radio; Switch overrides with its own padding) → `calc(${theme.spacing(1)} + 1px)`. **+1/unit step**, deliberately matching `FormControlLabel`'s `marginLeft: calc(spacing(-1) - 3px)` (-1/unit). The sum stays constant (`-2px`) at every density, so the checkbox/radio glyph stays in exactly the same visual position relative to the FormControlLabel edge regardless of `--mui-spacing`. Static styled wrapped via direct `({ theme }) => …` (codebase convention has been `memoTheme` for cached evaluation — call it out for follow-up).
