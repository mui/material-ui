...
[An accordion](https://material.io/archive/guidelines/components/accordions.html) is a lightweight container that may either stand alone or be connected to a larger surface, such as a card.
...
Extend the default panel behavior to create an accordion with the `Accordion` component.
...
In order to put an action such as a `Checkbox` or a button inside of the `AccordionSummary`, you need to stop the propagation of the focus and click events to prevent the panel from
...
The content of Accordions is mounted by default even if the panel is not expanded.
