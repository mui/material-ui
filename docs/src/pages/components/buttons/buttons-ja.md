...
One frequent use case is to perform navigation on the client only, without an HTTP round-trip to the server. `ButtonBase` コンポーネントはこういった使用法を扱うために`component` プロパティを提供します。 Here is a [more detailed guide](/guides/routing/#button).
