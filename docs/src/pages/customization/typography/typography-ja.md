...
> ⚠️ フォントサイズを変えるとアクセシビリティに悪影響を及ぼす可能性があります ♿️ ほとんどのブラウザはデフォルトの16ピクセルのサイズで一致していますが、ユーザーはそれを変更することができます。 例えば、視力に障害のある人は、ブラウザのデフォルトフォントサイズをもっと大きなサイズに設定しているかもしれません。 ほとんどのブラウザはデフォルトの16ピクセルのサイズで一致していますが、ユーザーはそれを変更することができます。 例えば、視力に障害のある人は、ブラウザのデフォルトフォントサイズをもっと大きなサイズに設定しているかもしれません。

const theme = createMuiTheme({ typography: { // Tell Material-UI what's the font-size on the html element is. htmlFontSize: 10, }, });

```js
このユースケースでは、<code>htmlFontSize</code>テーマプロパティが提供されており、<code><html></code>要素のフォントサイズが何であるかをMaterial-UIに伝えます。 これは、計算されたフォントサイズが常に仕様と一致するように、<code>rem</code>の値を調整するために使用されます。
    htmlFontSize: 10,
  },
});
```テーマプロパティが提供されており、<html>要素のフォントサイズが何であるかをMaterial-UIに伝えます。 これは、計算されたフォントサイズが常に仕様と一致するように、remの値を調整するために使用されます。
    htmlFontSize: 10,
  },
});
</code>
