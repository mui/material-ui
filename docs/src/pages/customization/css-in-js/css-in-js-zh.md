...
Material-UI 的样式解决方案的核心处是使用 [JSS](https://github.com/cssinjs/jss)。 它是一个在运行时和服务器端工作工作的，[高性能的](https://github.com/cssinjs/jss/blob/next/docs/performance.md) JS 到 CSS 编译器。 它大约是8kb（缩小和压缩）并且可以通过[插件](https://github.com/cssinjs/jss/blob/next/docs/plugins.md)的API扩展。
