...
function getSuggestions(value: string, { showEmpty = false } = {}): [] || OptionType[] {
...
function DownshiftMultiple(props) {
  ...
  function handleKeyDown(event: React.KeyboardEvent<FormControl>) {
    if (selectedItem.length && !inputValue.length && event.key === 'Backspace') {
      setSelectedItem(selectedItem.slice(0, selectedItem.length - 1));
    }
  }

  function handleInputChange(event: React.KeyboardEvent<FormControl>) {
    setInputValue(event.target.value);
  }

  function handleChange(item: OptionType) {
