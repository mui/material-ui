...
**Note:** If you are using emotion and have a custom cache in your app, that one will override the one coming from MUI. In order for the injection order to still be correct, you need to add the prepend option. 下面是一个示例：
...
明确向提组件提供类名是不是太大费周章了？ [You can target the class names generated by MUI](/styles/advanced/#with-material-ui-core).
