...
export default function PlainCssPriority() {
  return (
    <CacheProvider value={cache}>
      {这里编写你的组件树。 现在您可以覆盖 Material-UI 的样式。 */}
    </CacheProvider>
  );
}
```
...
- [Create React App with styled-components](https://github.com/mui-org/material-ui/blob/HEAD/examples/create-react-app-with-styled-components)
- [Create React App with styled-components and typescript](https://github.com/mui-org/material-ui/blob/HEAD/examples/create-react-app-with-styled-components-typescript)
...
For example, if you attempt to style the `tooltip` generated by the [Tooltip](/components/tooltips/) component, you will need to pass along the `className` property to the element being rendered outside of it's DOM hierarchy. The following example shows a workaround:
...
export default function CssModulesPriority() {
  return (
    <CacheProvider value={cache}>
      {/* 这里编写你的组件 现在您可以覆盖 Material-UI 的样式。 */}
    </CacheProvider>
  );
}
```
