...
这个 Box 组件现在就支持了 [间距](/system/spacing/#api) 和 [颜色](/system/palette/#api)属性。 例如，你可以提供一个间距属性： `p` 和一个颜色属性： `color`
...
为了使 Box 组件更有用的，我们已预置了一些样式函数，下面是完整列表：
...
如果你已经在使用 `@material-ui/core`，你可以用我们预写好的 [ Box 组件](/utils/box/) (内部使用了 JSS):
...
**所有的**属性都是响应式的，我们支持3种不同的 Api。 如下的配置是默认断点(breakpoints)配置，不过同时它们是可定制的。
...
我们没有支持所有CSS属性。不过如果你想 支持一个新的属性 或者 改变的主题路径的前缀 也是可以的。
...
帮助函数 `style()` 也可用于根据主题来返回不同的属性给样式对象。 在这个例子中， `Variants` 属性可以是 `theme.typography` 对象的任意键(key)。
...
风格系统(styled-system)已经做得很好了， 这里解释了[它是如何工作的](https://github.com/jxnblk/styled-system/blob/master/docs/how-it-works.md#how-it-works)的。它可以帮助你对于这种"style function"概念建设一个模型。
...
`@material-ui/system` 从不同地方借鉴了一些想法和Api：

- [Tachyons](https://tachyons.io/) (2014年)是第一个促进了 [原子CSS模式(Atomic CSS pattern)](https://css-tricks.com/lets-define-exactly-atomic-css/)发展的CSS库。
- Tachyons (2017年) 跟随了 [Tailwind CSS](https://tailwindcss.com/)的脚步，他们让原子CSS更受欢迎。
...
- Pinterest, GitHub, and Segment.io 这些大公司都使用了相同的方式，只是有不同的偏好： 
