...
export default function TopLayoutSurvey(props) {
  ...
  const renderContent = (chunk, index) => {
    if (typeof chunk === 'string') {
      ...
      if (matchMarkdownChunk) {
        ...
        return subChunks.map((subChunk, subIndex) => {
          if (typeof subChunk === 'string') {
            return (
              <RichMarkdownElement
                key={`sub-markdown-${index}-${subIndex}`}
                demos={demos}
                demoComponents={demoComponents}
                srcComponents={srcComponents}
                renderedMarkdownOrDemo={subChunk}
                disableAd
          ...
          if (subChunk.chart) {
            ...
            try {
              ...
              try {
                ...
                return (
                  <div key={`sub-error-${index}-${subIndex}`} style={{ color: 'red' }}>
      ...
      // Render RichMarkdownElement for unmatched content
      return (
        <RichMarkdownElement
          key={index}
          demos={demos}
    ...
    console.warn('Non-string or object demo chunk encountered:', chunk);
...
if (process.env.NODE_ENV !== 'production') {
  TopLayoutSurvey.propTypes = exactProp(TopLayoutSurvey.propTypes);
}
