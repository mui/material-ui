...
import { blueDark, grey } from 'docs/src/modules/brandingTheme';
...
const DemoRoot = styled('div', {
  shouldForwardProp: (prop) => prop !== 'hiddenToolbar' && prop !== 'bg' && prop !== 'product',
})(({ theme, hiddenToolbar, bg, product }) => ({
  position: 'relative',
  outline: 0,
  margin: 'auto',
  display: 'flex',
  ...
  ...(bg === 'gradient' && {
    ...
    ...(product === 'joy-ui' && { padding: '0px !important' }),
    border: `1px solid`,
  ...
  return (
    <Root>
      ...
      >
        ...
        >
          {demoData.product === 'joy-ui' ? (
            <CssVarsProvider disableStyleSheetGeneration>
              <JoyModeObserver mode={mode} />
              {demoElement}
            </CssVarsProvider>
          ) : (
            demoElement
