...
This utility is built on top of the `styled()` module of [`@mui/styled-engine`](/material/guides/styled-engine/) and provides additional features.
...
2. It supports the theme's [`styleOverrides`](/material/customization/theme-components/#global-style-overrides) and [`variants`](/material/customization/theme-components/#adding-new-component-variants) to be applied, based on the `name` applied in the options (can be skipped).
