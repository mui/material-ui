...
> Este exemplo cria um objeto de tema para componentes customizados. If you intend to use some of the MUI's components you need to provide a richer theme structure using the `createTheme()` method. Head to the [theming section](/material/customization/theming/) to learn how to build your custom MUI theme.
...
Você pode [seguir o guia de renderização no servidor](/material/guides/server-rendering/) para um exemplo mais detalhado, ou leia o [`ServerStyleSheets` na documentação da API](/styles/api/#serverstylesheets).
