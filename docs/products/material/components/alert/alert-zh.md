...
您也可以使用 [过渡组件](/material/components/transitions/) ，如使用 `Collapse（展开）` 来实现提醒出现时的过渡效果。
...
你可以使用消息条（Snackbar）来显示一个带警告提示组件的 [toast](/material/components/snackbars/#customized-snackbars) 。
