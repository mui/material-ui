...
Google has created over 1,900 official Material icons, each in five different "themes" (see below). For each SVG icon, we export the respective React component from the `@mui/icons-material` package. Você pode [pesquisar na lista completa destes ícones](/material/components/material-icons/).
...
Se você precisa de um ícone SVG customizado (não disponível nos [ícones Material](/material/components/material-icons/)) você pode usar encapsular com `SvgIcon`. Este componente estende o elemento nativo `<svg>`:
