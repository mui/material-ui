...
const Carousel = React.forwardRef<HTMLDivElement, CarouselProps>(function Carousel(
  ...
  // Context value for sub-components - memoized to prevent unnecessary re-renders
  ...
  // Slot components - memoized to prevent unnecessary re-renders
  const RootSlot = React.useMemo(() => slots.root ?? CarouselRoot, [slots.root]);
  const SlidesSlot = React.useMemo(() => slots.slides ?? CarouselSlides, [slots.slides]);
  const SlideSlot = React.useMemo(() => slots.slide ?? CarouselSlide, [slots.slide]);
