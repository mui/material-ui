...
| <span class="prop-name">children</span> | <span class="prop-type">node</span> |  | The option elements to populate the select with (they **must** be direct descendants). Can be some `MenuItem` when `native` is false and `option` when `native` is true. |
