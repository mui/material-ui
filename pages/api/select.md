...
| <span class="prop-name">displayEmpty</span> | <span class="prop-type">bool</span> | <span class="prop-default">false</span> | If `true`, the selected item is displayed even if its value is empty. If you want to show a meaningful display, the `renderValue` property must be used to check for the case of an empty selected value and provide a custom render value. You can only use it when the `native` property is `false` (default). |
