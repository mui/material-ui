...
| <span class="prop-name">hiddenLabel</span> | <span class="prop-type">bool</span> | <span class="prop-default">false</span> | If `true`, the label will be hidden. This is used to increase density for a `FilledInput` with `margin="dense"`. Be sure to add `aria-label` to `input` if you're using this component directly instead of `TextField`. |
