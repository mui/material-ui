...
- **Color is resting-only (state caveat).** Hover/active rules reassign `--variant-*`, which sits *below* a user-set `--Button-bg` in the chain, so a custom `--Button-bg` does **not** darken on hover/active. By design: if you customize color, you own the states. The global remap path is unchanged — override `--mui-palette-*` to recolor with states intact.
