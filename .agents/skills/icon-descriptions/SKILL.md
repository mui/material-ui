...
The icon search on the docs site indexes both. The file is generated by looking at rendered icons, in two passes run by subagents, with `docs/scripts/iconDescriptions.mjs` doing everything around them.
...
- `synonyms.js` is maintained separately by `pnpm docs:mdicons:synonyms` (from Google's Material Symbols metadata). Do not edit it from this workflow.
