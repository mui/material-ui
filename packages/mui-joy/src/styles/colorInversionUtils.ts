...
import { ColorPaletteProp, ColorSystem } from './types';
...
export const skipInvertedColors = (
  theme: ThemeFragment & { colorSchemes: { light?: ColorSystem; dark?: ColorSystem } },
) => {
 ...
 * @returns (theme: ThemeFragment) => Record<DefaultColorPalette, CSSObject>
 */
export const applySoftInversion = (color: ColorPaletteProp) => (theme: ThemeFragment) => {
  const getCssVarDefault = createGetCssVar(theme.cssVarPrefix);
  const prefixVar = createPrefixVar(theme.cssVarPrefix);
  const getCssVar = (cssVar: string) => {
    const tokens = cssVar.split('-');
