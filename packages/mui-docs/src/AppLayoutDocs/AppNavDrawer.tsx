...
interface ProductDrawerButtonProps {
  productName: string;
  routes?: import('./MuiProductSelector').MuiProductSelectorRoutes;
}
...
function ProductIdentifier(props: ProductIdentifierProps) {
  const { name, metadata, versionSelector, routes } = props;
  return (
    <Box sx={{ flexGrow: 1 }}>
      <Typography
        sx={(theme) => ({
      ...
      <Box sx={{ display: 'flex' }}>
        <ProductDrawerButton productName={name} routes={routes} />
        {versionSelector}
      </Box>
    </Box>
  );
...
export default function AppNavDrawer(props: AppNavDrawerProps) {
  const { className, disablePermanent, mobileOpen, onClose, onOpen, routes } = props;
