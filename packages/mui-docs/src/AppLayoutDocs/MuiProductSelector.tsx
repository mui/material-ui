...
import SvgMuiLogomark from './icons/SvgMuiLogomark';
import SvgBaseUiLogo from './icons/SvgBaseUiLogo';
...
export interface MuiProductSelectorRoutes {
  materialDocs: string;
  baseDocs: string;
  joyDocs: string;
  systemDocs: string;
  dataGridOverview: string;
  datePickersOverview: string;
  chartsOverview: string;
  treeViewOverview: string;
}

function getCoreProducts(routes: MuiProductSelectorRoutes) {
  return [
    {
      id: 'material-ui',
      name: 'Material UI',
...
function getAdvancedProducts(routes: MuiProductSelectorRoutes) {
  return [
    {
      id: 'x-data-grid',
      name: 'Data Grid',
...
const MuiProductSelector = React.forwardRef(function MuiProductSelector(
  props: MenuListProps<'div'> & { routes: MuiProductSelectorRoutes },
  forwardedRef: React.ForwardedRef<HTMLDivElement>,
) {
  const { routes, ...other } = props;
  const pageContext = React.useContext(PageContext);
  const coreProducts = getCoreProducts(routes);
  const advancedProducts = getAdvancedProducts(routes);
