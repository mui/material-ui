...
describe('adaptV4Theme', () => {
  ...
  it("moves overrides to components' stylesOverrides", () => {
    const theme = {
      overrides: {
        MuiTable: {
          root: {
    ...
    expect(transformedTheme.components.MuiTable.stylesOverrides).to.deep.equal(
  ...
  it('merges props and overrides to components', () => {
    ...
    expect(transformedTheme.components.MuiButton.defaultProps).to.deep.equal(theme.props.MuiButton);
    expect(transformedTheme.components.MuiButton.stylesOverrides).to.deep.equal(
  ...
  it('merges partially migrated props and overrides from different components in appropriate key', () => {
    ...
    expect(transformedTheme.components.MuiButton.stylesOverrides).to.deep.equal(
      theme.stylesOverrides.MuiButton,
    );
