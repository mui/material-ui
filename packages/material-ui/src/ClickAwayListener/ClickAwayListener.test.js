...
describe('<ClickAwayListener />', () => {
  ...
  describe('prop: onClickAway', () => {
    it('should be call when clicking away', () => {
      ...
      const event = fireBodyMouseEvent('onClick');
    ...
    it('should not be call when clicking inside', () => {
      ...
      const event = new window.Event('onClick', { view: window, bubbles: true, cancelable: true });
    ...
    it('should not be call when defaultPrevented', () => {
      ...
      window.document.body.addEventListener('onClick', preventDefault);

      const event = new window.Event('onClick', { view: window, bubbles: true, cancelable: true });
      window.document.body.dispatchEvent(event);
      assert.strictEqual(handleClickAway.callCount, 0);

      window.document.body.removeEventListener('onClick', preventDefault);
    });
  });
