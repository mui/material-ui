...
describe('<Masonry />', () => {
  ...
  describe('hidden children', () => {
    it('should handle hidden first child correctly', function test() {
      if (/jsdom/.test(window.navigator.userAgent)) {
        this.skip();
      }

      const { getByTestId } = render(
        <Masonry columns={2} spacing={1} data-testid="masonry">
          <div style={{ display: 'none', height: '20px' }} data-testid="hidden-child" />
          <div style={{ height: '30px' }} data-testid="visible-child-1" />
          <div style={{ height: '40px' }} data-testid="visible-child-2" />
        </Masonry>,
      );

      const masonry = getByTestId('masonry');
      const visibleChild1 = getByTestId('visible-child-1');
      const visibleChild2 = getByTestId('visible-child-2');
      const hiddenChild = getByTestId('hidden-child');

      // Hidden child should not affect layout
      expect(window.getComputedStyle(hiddenChild).order).to.equal('');

      // Visible children should be properly ordered
      expect(window.getComputedStyle(visibleChild1).order).to.not.equal('');
      expect(window.getComputedStyle(visibleChild2).order).to.not.equal('');

      // Masonry should have correct columns (not collapsed to single column)
      const masonryStyle = window.getComputedStyle(masonry);
      expect(masonryStyle.display).to.equal('flex');
      expect(masonryStyle.flexDirection).to.equal('column');
    });

    it('should handle dynamically hidden children', function test() {
      if (/jsdom/.test(window.navigator.userAgent)) {
        this.skip();
      }
      ...
      const { rerender, getByTestId } = render(
        <Masonry columns={2} spacing={1}>
          <div style={{ height: '20px' }} data-testid="child-1" />
          <div style={{ height: '30px' }} data-testid="child-2" />
        </Masonry>,
      );

      // Verify initial state - both children should have orders
      expect(window.getComputedStyle(getByTestId('child-1')).order).to.not.equal('');
      expect(window.getComputedStyle(getByTestId('child-2')).order).to.not.equal('');

      // Hide the first child
      rerender(
        <Masonry columns={2} spacing={1}>
          <div style={{ display: 'none', height: '20px' }} data-testid="child-1" />
          <div style={{ height: '30px' }} data-testid="child-2" />
        </Masonry>,
      );

      // Layout should still work with hidden first child
      const hiddenChildOrder = window.getComputedStyle(getByTestId('child-1')).order;
      const visibleChildOrder = window.getComputedStyle(getByTestId('child-2')).order;
      
      // Hidden child should not have an order
      expect(hiddenChildOrder).to.equal('');
      // Visible child should still have an order
      expect(visibleChildOrder).to.not.equal('');
    });

    it('should handle all children being hidden', function test() {
      if (/jsdom/.test(window.navigator.userAgent)) {
        this.skip();
      }
      ...
      const { container } = render(
        <Masonry columns={2} spacing={1}>
          <div style={{ display: 'none' }} data-testid="child-1" />
          <div style={{ display: 'none' }} data-testid="child-2" />
        </Masonry>,
      );

      // Should not throw an error
      expect(() => container.querySelector('[class*=MuiMasonry]')).to.not.throw();
    });
  });
