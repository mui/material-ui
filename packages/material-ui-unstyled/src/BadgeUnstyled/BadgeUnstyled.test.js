...
describe('<BadgeUnstyled />', () => {
  ...
  it('renders children', () => {
    const { container, getByTestId } = render(
      <BadgeUnstyled className="testClassName" {...defaultProps} />,
    );
  ...
  describe('prop: color', () => {
    it('should have the colorPrimary class when color="primary"', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} color="primary" />);
    ...
    it('should have the colorSecondary class when color="secondary"', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} color="secondary" />);
      expect(findBadge(container)).to.have.class(classes.colorSecondary);
    });

    it('should have the colorError class when color="error"', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} color="error" />);
  ...
  describe('prop: invisible', () => {
    it('should default to false', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} />);
    ...
    it('should render without the invisible class when set to false', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} invisible={false} />);
      expect(findBadge(container)).to.not.have.class(classes.invisible);
    });

    it('should render with the invisible class when set to true', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} invisible />);
    ...
    it('should render with the invisible class when empty and not dot', () => {
      let container;
      container = render(<BadgeUnstyled {...defaultProps} badgeContent={null} />).container;
      expect(findBadge(container)).to.have.class(classes.invisible);
      container = render(<BadgeUnstyled {...defaultProps} badgeContent={undefined} />).container;
      expect(findBadge(container)).to.have.class(classes.invisible);
      container = render(<BadgeUnstyled {...defaultProps} badgeContent={undefined} variant="dot" />)
        .container;
      expect(findBadge(container)).to.not.have.class(classes.invisible);
    });
  });
  ...
  describe('prop: showZero', () => {
    it('should default to false', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} badgeContent={0} />);
      expect(findBadge(container)).to.have.class(classes.invisible);
    });

    it('should render without the invisible class when false and badgeContent is not 0', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} showZero />);
      expect(findBadge(container)).to.not.have.class(classes.invisible);
    });

    it('should render without the invisible class when true and badgeContent is 0', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} badgeContent={0} showZero />);
      expect(findBadge(container)).to.not.have.class(classes.invisible);
    });
  ...
  describe('prop: variant', () => {
    ...
    it('should render with the standard class when variant="standard"', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} variant="standard" />);
      expect(findBadge(container)).to.have.class(classes.badge);
      expect(findBadge(container)).to.not.have.class(classes.dot);
    });

    it('should not render badgeContent when variant="dot"', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} variant="dot" />);
      expect(findBadge(container)).to.have.class(classes.badge);
  ...
  describe('prop: max', () => {
    it('should default to 99', () => {
      const { container } = render(<BadgeUnstyled {...defaultProps} badgeContent={100} />);
