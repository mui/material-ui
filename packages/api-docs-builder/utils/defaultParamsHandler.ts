...
function getRenderBody(hookDefinition: NodePath, importer: Importer): NodePath {
  const value = resolveToValue(hookDefinition, importer);
  return value.get('body', 'body');
}

function getParamsPath(functionBody: NodePath): NodePath | undefined {
  ...
  functionBody
    ...
    .forEach((path: NodePath) => {
      ...
      if (
        declaratorPath.get('init', 'name').value === 'parameters' &&
...
const defaultParamsHandler: Handler = (documentation, hookDefinition, importer) => {
  const renderBody = getRenderBody(hookDefinition, importer);
  const params = getParamsPath(renderBody);
  if (params !== undefined) {
    getDefaultValuesFromProps(params.get('properties'), documentation, importer);
  }
};

export default defaultParamsHandler;
