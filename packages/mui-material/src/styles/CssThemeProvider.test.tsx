...
describe('CssThemeProvider', () => {
  ...
  it('throws when the theme was not created with cssVariables: true', () => {
    ...
    expect(() =>
      render(<CssThemeProvider theme={theme}>{null}</CssThemeProvider>),
    ).toErrorDev(['MUI: CssThemeProvider requires a theme created with `cssVariables: true`']);
  });

  it('throws when passed a theme function', () => {
    const themeFn = () => createTheme({ cssVariables: true });
    expect(() =>
      render(<CssThemeProvider theme={themeFn}>{null}</CssThemeProvider>),
    ).toErrorDev(['MUI: CssThemeProvider does not accept a theme function']);
  });
