...
import VariableIcon, { variableIconClasses as classes } from '@mui/material/SvgIcon';
...
describe('<VariableIcon />', () => {
  ...
  it('renders children by default', () => {
    const { container, queryByTestId } = render(<VariableIcon>{path}</VariableIcon>);
  ...
  describe('prop: titleAccess', () => {
    it('should be able to make an icon accessible', () => {
      const { container, queryByText } = render(
        <VariableIcon title="Go to link" titleAccess="Network">
          {path}
        </VariableIcon>,
      );
  ...
  describe('prop: fontSize', () => {
    it('should be able to change the fontSize', () => {
      const { container } = render(<VariableIcon fontSize="inherit">{path}</VariableIcon>);
  ...
  describe('prop: inheritViewBox', () => {
    ...
    it('should render with the default viewBox if neither inheritViewBox nor viewBox are provided', () => {
      const { container } = render(<VariableIcon component={CustomSvg} />);
    ...
    it('should render with given viewBox if inheritViewBox is not provided', () => {
      const { container } = render(<VariableIcon component={CustomSvg} viewBox="0 0 30 30" />);
      expect(container.firstChild).to.have.attribute('viewBox', '0 0 30 30');
    });

    it("should use the custom component's viewBox if true", () => {
      const { container } = render(<VariableIcon component={CustomSvg} inheritViewBox />);
  ...
  it('should not override internal ownerState with the ownerState passed to the icon', function test() {
    ...
    const { container } = render(
      <VariableIcon ownerState={{ fontSize: 'large' }}>{path}</VariableIcon>,
    );
