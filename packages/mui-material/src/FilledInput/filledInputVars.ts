 ...
 * size). The floating label's resting/shrunk Y are separate seams in
 * `inputLabelVars` (`filledRestY`/`filledShrinkY`) — set by the presets (no
 * clean formula from `topPad`, the offsets are hand-tuned per size), consumed
