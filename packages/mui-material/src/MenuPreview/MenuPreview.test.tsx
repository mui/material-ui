...
describe('<MenuPreview />', () => {
  ...
  it('supports render, slotProps, classes, styleOverrides, and variants', async () => {
  ...
  it('composes popup className callbacks', async () => {
    const { user } = render(
      <MenuPreview>
        <MenuPreviewTrigger>Options</MenuPreviewTrigger>
        <MenuPreviewPopup
          ...
          slotProps={{
            popup: {
              className: (state) => `popup-side-${state.side}`,
            },
          }}
        >
          <MenuPreviewItem>Profile</MenuPreviewItem>
