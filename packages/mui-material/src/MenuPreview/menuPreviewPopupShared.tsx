...
import { resolveSlotProps, resolveStateClassName, SlotProps } from './menuPreviewUtils';

type ExternalSlotProps<Props> = Partial<Props> & Record<string, any>;
...
export interface MenuPreviewPopupPublicProps {
   ...
   */
  className?: string | ((state: MenuPreviewPopupState) => string) | undefined;
  /**
   * Styles applied to the Base UI popup element.
   */
  style?: React.CSSProperties | ((state: MenuPreviewPopupState) => React.CSSProperties) | undefined;
  /**
...
export const MenuPreviewPopupBase = React.forwardRef(function MenuPreviewPopupBase<OwnerState>(
  ...
  const popupClassName = (state: BaseMenu.Popup.State) =>
    clsx(
      classes?.root,
      resolveStateClassName(className, state),
      resolveStateClassName(resolvedPopupClassName, state),
    );
  const popupRender = resolvedPopupRender ?? (
    <PopupSlot {...appendOwnerState(PopupSlot, {}, ownerState)} />
  );
