...
- A fixed-pixel container in the surrounding layout could cause clipping at 200%. Covered by a Playwright test at 200% text size.
...
- Real failures usually come from the surrounding layout, such as a wide non-wrapping row of labeled radios or a fixed-width container (a default `row` group still wraps, since `FormGroup` keeps `flex-wrap: wrap`). Covered by a Playwright test at a 320px viewport.
...
- Whether a long label clips depends on the author's container. Covered by a Playwright test applying the WCAG text-spacing overrides.
