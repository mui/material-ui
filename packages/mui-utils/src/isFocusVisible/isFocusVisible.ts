/**
import { isJsdom } from '@mui/internal-test-utils/env'; * Returns a boolean indicating if the event's target has :focus-visible
 */
export default function isFocusVisible(element: Element): boolean {
  try {
    ...
    if (process.env.NODE_ENV !== 'production' && !isJsdom()) {
      console.warn(
        [
